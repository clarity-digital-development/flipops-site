...
**Status:** DONE (2026-06-12 — package built in `docs/investor/` after a 5-lane read-only audit grounded every claim in code. (1) **Marketing integrity fix** (`37201c3`): the live site claimed a "157M+ property database", "Powered by CoreLogic" (NOT integrated), "all 50 states", "nationwide" — ALL fabricated. Scoped 10 claims across trust-bar/feature-tabs/FAQ/compare/demo/arv-calculator to verified FL truth (10,998,035 parcels, 67 counties, self-scraped). Fixed fabricated `netToSeller` 0.9x → null. (2) **`SCRAPER-RUNBOOKS.md`** — per-source ops runbook, 7 families, honest live/dormant/disabled status. (3) **`TCPA-SUBSTANTIATION.md`** — honest audit: control architecture is real (chokepoint + fail-closed gates + 50-state quiet-hours table + audit rows) but consent-capture/DNC/STOP are stubbed → defensible claim is "architecture in place, integrations pending," NOT "safe by construction"; remediation path documented. (4) **`DEMO-SCRIPT.md`** — the 6-step walkthrough (Steps 1-4,6 verified real-data-backed) + a pre-flight checklist (seed a Dade/Broward lead, never click "Try with a sample property") that satisfies the zero-fabricated-numbers bar. (5) **`INVESTOR-DECK.md`** — draft with every number confidence-labeled [verified]/[competitor-asserted]/[aspirational]; pricing reconciled to $99/$299/$599 (founder decision) across deck + marketing site (Core $149→$99, Scale $549→$599) + savings calculator. **Founder follow-ups:** fill the deck's "ask" slide; wire the Stripe backend tiers to $99/$299/$599; the deck's data-moat numbers strengthen as the SDF backfill lands.) · **Effort:** ~1 week · **Deps:** everything above (it's the capstone)
**What/How:**
