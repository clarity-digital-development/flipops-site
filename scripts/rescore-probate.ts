  ...
  const sql = `
    ...
    FROM (
      SELECT
        ...
        (ARRAY_AGG(pc."caseNumber" ORDER BY pc."prAppointedAt" DESC NULLS LAST, pc."filedAt" DESC NULLS LAST, pc."caseNumber" DESC))[1] AS "primaryCaseNumber",
        (ARRAY_AGG(pc."decedentName" ORDER BY pc."prAppointedAt" DESC NULLS LAST, pc."filedAt" DESC NULLS LAST, pc."caseNumber" DESC))[1] AS "decedentName",
        (ARRAY_AGG(pc."personalRepresentative" ORDER BY pc."prAppointedAt" DESC NULLS LAST, pc."filedAt" DESC NULLS LAST, pc."caseNumber" DESC))[1] AS "personalRepresentative",
        (ARRAY_AGG(pc."attorneyName" ORDER BY pc."prAppointedAt" DESC NULLS LAST, pc."filedAt" DESC NULLS LAST, pc."caseNumber" DESC))[1] AS "attorneyName",
        (ARRAY_AGG(pc."attorneyAddress" ORDER BY pc."prAppointedAt" DESC NULLS LAST, pc."filedAt" DESC NULLS LAST, pc."caseNumber" DESC))[1] AS "attorneyAddress",
        (ARRAY_AGG(pc."caseTypeCode" ORDER BY pc."prAppointedAt" DESC NULLS LAST, pc."filedAt" DESC NULLS LAST, pc."caseNumber" DESC))[1] AS "caseTypeCode",
        MIN(pc."filedAt")                                                    AS "filedAt",
        MAX(pc."prAppointedAt")                                              AS "prAppointedAt",
        MAX(pc."decedentDateOfDeath")                                        AS "dateOfDeath",
        (ARRAY_AGG(pc."status" ORDER BY pc."prAppointedAt" DESC NULLS LAST, pc."filedAt" DESC NULLS LAST, pc."caseNumber" DESC))[1] AS "status",
